...
//$data=file_get_contents("http://en.wikipedia.org/wiki/" . $_GET['cityname'] );
...
if($start!=false)
{
	...
	$formatvalue=mb_substr($semi1,0,$start2,'UTF-8');
	$test=mb_strpos($formatvalue,"°C)",0,'UTF-8');
...
$lim=sizeof($set);
	...
	while($i < $lim)
	{
		...
		if($test==false)
			...
			$semi=strval(round((floatval($semi)-32)*0.55555,2));
	...
	$test=mb_strpos($data,"Average low</abbr> °F",0,'UTF-8');
...
if($start!=false)
{
	...
	while($i < $lim)
	{
		...
		{
			$semi=strval(round((floatval($semi)-32)*0.5555,2));
		}
		array_push($stack,($semi));
		$i=$i+1;
		if($i==13)
